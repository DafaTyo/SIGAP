...
from app.core.exceptions import VendorNotFound, PermissionDenied, NotFoundError, IdempotencyConflict
...
async def register_vendor(db: AsyncSession, dto: VendorCreate) -> VendorRead:
    try:
        ...
        raise IdempotencyConflict(
            detail=f"Vendor dengan nama_usaha '{dto.nama_usaha}' sudah terdaftar"
        ) from exc
...
async def get_sio(db: AsyncSession, vendor_id: uuid.UUID) -> SIODigitalRead:
    ...
    if vendor.status != "verified":
        raise NotFoundError(detail="Vendor belum terverifikasi, SIO belum tersedia")
